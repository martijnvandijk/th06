...
WebSocket::WebSocket(TCPSocket* sock)
throw (WebSocketException): data(NULL), datalen(0), closed(false), closing(false)
{
	this->sock = sock;
	performHandshake();
...
//als het wel een final frame is, verwerk het frame op een wijze die
